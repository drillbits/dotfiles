...
Terminal workspace manager for coding agents, installed via mise. Config at `.config/herdr/config.toml` holds only deviations from herdr defaults: prefix aligned with tmux (`Ctrl+t`), `prefix+Space` for next tab, pane keys matching tmux (`prefix+|` split side by side, `prefix+o` next pane; the notification-target action moves to `prefix+shift+o` to free `o`), toast notifications delivered through the outer terminal (works over SSH, same idea as the OSC 52 clipboard), and the `dracula` theme to match wezterm. Two `[theme.custom]` tokens are lifted to dracula's overlay1 (`#828cb4`) because dracula draws them at background level and they vanish on wezterm's translucent background: `overlay0` (unfocused split-pane frames; the focused frame uses `accent`) and `surface_dim` (the sidebar/pane divider and the rule between the spaces and agents sections). Runs alongside tmux, not nested inside it — nesting requires double-pressing the prefix and breaks terminal-delivered notifications. A running server picks up config edits with `herdr server reload-config` (or `prefix+shift+r`); try candidate settings first with `HERDR_CONFIG_PATH=<file>` in a throwaway `herdr --session <name>` so the default session stays untouched.
