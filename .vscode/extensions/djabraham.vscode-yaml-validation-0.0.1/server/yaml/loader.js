...
function storeMappingPair(state, _result, keyTag, keyNode, valueNode, parent) {
    var index, quantity;
    if (keyNode == null) {
        return;
    }
    ...
    if (null === _result) {
        _result = {
            ...
            errors: [],
            properties: [],
            kind: YamlInterface.Kind.MAP,
        };
    }
    if ('tag:yaml.org,2002:merge' === keyTag) {
        throw new Error("Should not happen (tag:yaml.org,2002:merge)"); // TODO: LOOK IN IT
    }
    else {
        var mapping = YamlInterface.newMapping(keyNode, valueNode);
        ...
        _result.properties.push(mapping);
        _result.end = valueNode ? valueNode.end : keyNode.end + 1; //FIXME.workaround should be position of ':' indeed
    }
    return _result;
}
...
function writeFoldedLines(state, scalar, count) {
    if (1 === count) {
        ...
        scalar.buffer += common.repeat('\n', count - 1);
    }
}
function readPlainScalar(state, nodeIndent, withinFlowCollection) {
    var preceding, following, captureStart, captureEnd, hasPendingContent, _line, _lineStart, _lineIndent, _kind = state.kind, _result = state.result, ch;
    var state_result = YamlInterface.newScalar();
...
function readSingleQuotedScalar(state, nodeIndent) {
    ...
    var scalar = YamlInterface.newScalar();
    state.kind = 'scalar';
    state.result = scalar;
    scalar.start = state.position;
    state.position++;
    captureStart = captureEnd = state.position;
    ...
    while (0 !== (ch = state.input.charCodeAt(state.position))) {
        if (0x22 /* " */ === ch) {
            ...
            if (is_EOL(ch)) {
                ...
                scalar.buffer += simpleEscapeMap[ch];
                state.position++;
            }
                ...
                scalar.buffer += charFromCodepoint(hexResult);
                state.position++;
            }
            else {
...
function readFlowCollection(state, nodeIndent) {
    ...
    if (ch === 0x5B /* [ */) {
        terminator = 0x5D; /* ] */
        isMapping = false;
        _result = YamlInterface.newItems();
        _result.start = state.position;
    }
    else if (ch === 0x7B /* { */) {
        terminator = 0x7D; /* } */
        isMapping = true;
        _result = YamlInterface.newMap();
        _result.start = state.position;
    }
    else {
        return false;
    ...
    while (0 !== ch) {
        ...
        if (isMapping) {
            ...
            var mp = storeMappingPair(state, null, keyTag, keyNode, valueNode, _result);
            mp.parent = _result;
            _result.items.push(mp);
        }
        else {
            keyNode.parent = _result;
            _result.kind = YamlInterface.Kind.SEQ;
            _result.items.push(keyNode);
        }
        _result.end = state.position + 1 /*need to add one more char*/;
        skipSeparationSpace(state, true, nodeIndent);
        ch = state.input.charCodeAt(state.position);
        if (0x2C /* , */ === ch) {
            readNext = true;
...
function readBlockScalar(state, nodeIndent) {
    ...
    var sc = YamlInterface.newScalar();
    state.kind = 'scalar';
    state.result = sc;
    sc.start = state.position;
    while (0 !== ch) {
        ch = state.input.charCodeAt(++state.position);
        if (0x2B /* + */ === ch || 0x2D /* - */ === ch) {
...
function readBlockSequence(state, nodeIndent) {
    var _line, _tag = state.tag, _anchor = state.anchor, _result = YamlInterface.newItems(), following, detected = false, ch;
    if (null !== state.anchor) {
        _result.anchorId = state.anchor;
        state.anchorMap[state.anchor] = _result;
    }
    ...
    while (0 !== ch) {
        ...
        state.result.name = _result.items.length.toString();
...
function readBlockMapping(state, nodeIndent, flowIndent) {
    var following, allowCompact, _line, _tag = state.tag, _anchor = state.anchor, _result = YamlInterface.newMap(), keyTag = null, keyNode = null, valueNode = null, atExplicitKey = false, detected = false, ch;
    _result.start = state.position;
    if (null !== state.anchor) {
        _result.anchorId = state.anchor;
        state.anchorMap[state.anchor] = _result;
...
function readAlias(state) {
    ...
    state.result = YamlInterface.newAnchorRef(alias, _position, state.position, state.anchorMap[alias]);
    skipSeparationSpace(state, true, -1);
    return true;
}
function composeNode(state, parentIndent, nodeContext, allowToSeek, allowCompact) {
    ...
    if (null !== state.tag && '!' !== state.tag) {
        if (state.tag == "!include") {
            if (!state.result) {
                state.result = YamlInterface.newScalar();
                state.result.start = state.position;
                state.result.end = state.position;
                throwError(state, "!include without value");
            }
            state.result.kind = YamlInterface.Kind.INCLUDE_REF;
        }
        else if ('?' === state.tag) {
            for (typeIndex = 0, typeQuantity = state.implicitTypes.length; typeIndex < typeQuantity; typeIndex += 1) {
                ...
                var vl = state.result['buffer'];
                if (!vl)
                    console.log("NULL: ", state.result);
                if (type.resolve(vl)) {
                    // setScalerJsonProps(state.result, type.construct(state.result['buffer']));
                    state.result.valueNative = type.construct(state.result['buffer']);
                    state.tag = type.tag;
                    if (null !== state.anchor) {
                        state.result.anchorId = state.anchor;
                        state.anchorMap[state.anchor] = state.result;
...
function exportResult(state) {
    ...
    var value = undefined;
    if (s.valueNative != null) {
        value = s.valueNative;
        delete s.valueNative;
    }
    else {
        value = s.buffer;
    }
    ...
    if (vtype === 'number') {
        s.type = vtype;
        // the 32bit shift (>>) truncates any bits beyond 32
