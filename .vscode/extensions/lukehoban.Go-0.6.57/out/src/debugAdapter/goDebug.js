    ...
    constructor(remotePath, port, host, program, launchArgs) {
        ...
        this.connection = new Promise((resolve, reject) => {
            ...
            if (!!launchArgs.noDebug && mode === 'debug' && !isProgramDirectory) {
                this.debugProcess = child_process_1.spawn(goPath_1.getGoRuntimePath(), ['run', program], { env: finalEnv });
            ...
            if (launchArgs.envFile) {
                try {
                    const buffer = goPath_1.stripBOM(FS.readFileSync(launchArgs.envFile, 'utf8'));
                    buffer.split('\n').forEach(line => {
