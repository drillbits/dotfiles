...
class WorkspaceSymbols {
    ...
    registerCommands() {
        this.disposables.push(vscode.commands.registerCommand(constants_1.Commands.Build_Workspace_Symbols, this.buildWorkspaceSymbols.bind(this)));
    }
            ...
            return promise.catch(reason => {
                ...
                return new Promise((resolve, reject) => {
                    vscode.window.showErrorMessage('CTags needs to be installed to get support for Python workspace symbols', 'Install', `Don't ask again`).then(item => {
                        switch (item) {
                            case 'Install': {
                                this.installer.installProduct(installer_1.Product.ctags).then(() => {
